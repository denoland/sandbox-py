...
class Volumes:
    ...
    def create(self, options: VolumeInit) -> Volume:
        ...
        result = self._client._bridge.run(
            self._client._async.post("/api/v2/volumes", params)
        )
    ...
    def get(self, id_or_slug: str) -> Volume | None:
        ...
        result = self._client._bridge.run(
            self._client._async.get_or_none(f"/api/v2/volumes/{id_or_slug}")
        )
        if result is None:
            return None
        raw_result = convert_to_snake_case(result)
    ...
    def list(
        ...
        paginated = self._client._bridge.run(
            self._client._async.get_paginated("/api/v2/volumes", cursor=None, params=options)
        )
    ...
    def delete(self, id_or_slug: str) -> None:
        ...
        self._client._bridge.run(
            self._client._async.delete(f"/api/v2/volumes/{id_or_slug}")
        )

    def snapshot(self, id_or_slug: str, init: SnapshotInit) -> Snapshot:
        """Create a snapshot of a volume by ID or slug."""
        result = self._client._bridge.run(
            self._client._async.post(f"/api/v2/volumes/{id_or_slug}/snapshot", init)
        )
        raw_result = convert_to_snake_case(result)
...
class AsyncVolumes:
    ...
    async def list(
        ...
        """List volumes."""
        return await self._client.get_paginated("/api/v2/volumes", cursor=None, params=options)
